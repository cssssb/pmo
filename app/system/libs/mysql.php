...
final class mysql {
	 ...
	 * @param $limit 		返回结果范围[例：@0或@0,@0 默认为空]
	 ...
	 * 						为字符串时[例：`name`='phpcms',`hits`=`hits`+@]。
	 *						为数组时[例: array('name'=>'phpcms','password'=>'@23456')]
	 *						数组可使用array('name'=>'+=@', 'base'=>'-=@');程序会自动解析为`name` = `name` + @, `base` = `base` - @
	 * @param $table 		数据表
	...
	public function halt($message = '', $sql = '') {
		if($this->config['debug']) {
			...
			$msg = $this->errormsg;
			echo '<div style="font-size:@2px;text-align:left; border:@px solid #9cc9e0; padding:@px 4px;color:#000000;font-family:Arial, Helvetica,sans-serif;"><span>'.$msg.'</span></div>';
			exit;
		} else {
			return false;
		}
